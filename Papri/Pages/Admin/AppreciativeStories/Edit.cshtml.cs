...
public class EditModel : PageModel
{
    ...
    [BindProperty] public AppreciativeStory Item { get; set; } = default!;
    [BindProperty] public IFormFile? ImageFile { get; set; }

    public async Task<IActionResult> OnGetAsync(int id)
    {
        var entity = await _db.AppreciativeStories.FindAsync(id);
    ...
    public async Task<IActionResult> OnPostAsync(int id)
    {
        ...
        {
            var current = await _db.AppreciativeStories.AsNoTracking().FirstOrDefaultAsync(s => s.Id == id);
        ...
        if (ImageFile is { Length: > 0 })
        {
            try
            {
                var newPath = await _files.SaveAsync(ImageFile, "appreciative-stories", AllowedFiles.Images, AllowedFiles.ImageMaxBytes);
        ...
        TempData["StatusMessage"] = "Appreciative story updated.";
